...
public class Join {
    ...
    public void onServerConnected(ServerConnectedEvent event) {
        ...
        if (event.getPreviousServer().isPresent()) {
            ...
            if (isVanished) {
                for (Player players : i.server.getAllPlayers())
                    if (players.hasPermission("silverstone.moderator")) players.sendMessage(Component.text()
                        .append(
                            displayName.colorIfAbsent(NamedTextColor.AQUA),
        ...
        if (event.getPlayer().getProtocolVersion().getProtocol() != i.fileManager.files.get(CONFIG).node(
            "current-protocol-version").getInt()) i.server.getScheduler().buildTask(
        ...
        if (!userExists) {
            int staff = 0;
            for (Player players : i.server.getAllPlayers()) {
                players.sendMessage(Component.text("Welcome to Silverstone, ", NamedTextColor.GREEN).append(
