    ...
    public void execute(final Invocation invocation) {
        ...
        if (args.length == 0) {
            if (sender.hasPermission("silverstone.nickname.others"))
                if (sender.hasPermission("silverstone.nickname.colors")) sender.sendMessage(
            ...
            else if (sender.hasPermission("silverstone.nickname.colors")) sender.sendMessage(
        ...
        if (args[0].equalsIgnoreCase("colors") && (sender.hasPermission(
            "silverstone.nickname.colors") || sender.hasPermission("silverstone.nickname.formats"))) {
        ...
        if (sender.hasPermission("silverstone.nickname.colors"))
        ...
        if (!sender.hasPermission("silverstone.nickname.bypass.alphanumeric"))
        ...
        if (new NicknameUtils(i).setNickname(player, sender, nickname,
            MiniMessage.miniMessage().stripTags(args[0]))) return;

        if (differentTarget) {
            sender.sendMessage(Component.text("You have set ", NamedTextColor.GREEN)
                .append(Component.text(player.getUsername())).append(Component.text("'s nickname to "))
