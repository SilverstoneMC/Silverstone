...
public class WarnCommand extends Command implements TabExecutor {
    ...
    public void execute(CommandSender sender, String[] args) {
        ...
        if (!(sender instanceof ProxiedPlayer))
            switch (args.length) {
                case 0, 1 -> sender.sendMessage(TextComponent.fromLegacyText(ChatColor.RED + "/warn <player> <reason>"));

                case 2 -> {
                    // Warn the targeted player
                    if (checkIfValidReason(args[1])) {
                        plugin.getProxy().getPluginManager().dispatchCommand(plugin.getProxy().getConsole(), "reasons");
                        return;
                    }
    ...
    private boolean checkIfValidReason(String reason) {
        ArrayList<String> reasonList = new ArrayList<>(SilverstoneWarnings.config.getSection("reasons").getKeys());
