...
public class BaseCommand extends Command implements TabExecutor {
    ...
    public void execute(CommandSender sender, String[] args) {
        ...
        // Reload
        if (args.length > 0) if (args[0].equalsIgnoreCase("reload")) if (sender.hasPermission("silverstone.admin")) {
            ...
            sender.sendMessage(TextComponent.fromLegacyText(ChatColor.GREEN + "SilverstoneWarnings reloaded!"));
        } else
            sender.sendMessage(TextComponent.fromLegacyText(ChatColor.RED + "You don't have permission to do that!"));

            // remove <reason> <player>
        else if (args[0].equalsIgnoreCase("remove")) {
            if (args.length < 3) {
                sender.sendMessage(TextComponent.fromLegacyText(ChatColor.RED + "/ssw remove <reason> <player>"));
                return;
            }
            ...
            if ((count - 1) < 0)
                sender.sendMessage(TextComponent.fromLegacyText(ChatColor.translateAlternateColorCodes('&', "&7" + username + " &calready has 0 &7" + args[1] + " &cwarnings.")));
            else {
            ...
            if (args.length < 3) {
                sender.sendMessage(TextComponent.fromLegacyText(ChatColor.RED + "/ssw clear <[reason]|all> <player>"));
                return;
            }
            ...
            if (args[1].equalsIgnoreCase("all")) {
                ...
                plugin.getLogger()
                        .info(ChatColor.translateAlternateColorCodes('&', "&7" + sender.getName() + " &ccleared all of &7" + username + "'s &cwarnings"));

                // Message staff
                for (ProxiedPlayer online : plugin.getProxy().getPlayers())
                    if (online.hasPermission("sswarnings.warn"))
                        online.sendMessage(TextComponent.fromLegacyText(ChatColor.translateAlternateColorCodes('&', "&7" + sender.getName() + " &ccleared all of &7" + username + "'s &cwarnings")));

                // Message player if online and command not silent
                try {
                    if (!args[3].equalsIgnoreCase("-s"))
                        if (player != null)
                            player.sendMessage(TextComponent.fromLegacyText(ChatColor.translateAlternateColorCodes('&', "&7" + sender.getName() + " &ccleared all your warnings.")));
                } catch (ArrayIndexOutOfBoundsException ignored) {
                    if (player != null)
                        player.sendMessage(TextComponent.fromLegacyText(ChatColor.translateAlternateColorCodes('&', "&7" + sender.getName() + " &ccleared all your warnings.")));
                }
                ...
                EmbedBuilder embed = new EmbedBuilder();
                embed.setAuthor(sender.getName() + " cleared all of " + username + "'s warnings", null, "https://crafatar.com/avatars/" + uuid + "?overlay=true");
                embed.setColor(new Color(42, 212, 85));
                warningChannel.sendMessageEmbeds(embed.build()).queue();
                ...
                plugin.getLogger()
                        .info(ChatColor.translateAlternateColorCodes('&', "&7" + sender.getName() + " &ccleared all &7" + args[1] + " &cwarnings from &7" + username));

                // Message staff
                for (ProxiedPlayer online : plugin.getProxy().getPlayers())
                    if (online.hasPermission("sswarnings.warn"))
                        online.sendMessage(TextComponent.fromLegacyText(ChatColor.translateAlternateColorCodes('&', "&7" + sender.getName() + " &ccleared all &7" + args[1] + " &cwarnings from &7" + username)));

                // Message player if online and command not silent
                try {
                    if (!args[3].equalsIgnoreCase("-s"))
                        if (player != null)
                            player.sendMessage(TextComponent.fromLegacyText(ChatColor.translateAlternateColorCodes('&', "&7" + sender.getName() + " &ccleared all your &7" + args[1] + " &cwarnings.")));
                } catch (ArrayIndexOutOfBoundsException ignored) {
                    if (player != null)
                        player.sendMessage(TextComponent.fromLegacyText(ChatColor.translateAlternateColorCodes('&', "&7" + sender.getName() + " &ccleared all your &7" + args[1] + " &cwarnings.")));
                }
                ...
                EmbedBuilder embed = new EmbedBuilder();
                embed.setAuthor(sender.getName() + " cleared all '" + args[1] + "' warnings from " + username, null, "https://crafatar.com/avatars/" + uuid + "?overlay=true");
                embed.setColor(new Color(42, 212, 85));
                warningChannel.sendMessageEmbeds(embed.build()).queue();
            }
        }
